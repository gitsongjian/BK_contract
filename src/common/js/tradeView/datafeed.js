...
import  datafeedConfig from './datafeedConfig.js'
import datafeedUtil from './datafeedUtil.js'
const { configJSON, symbolResolveJSON } = datafeedConfig;

let Datafeeds = {};
//var getBarTimes = 0;
Datafeeds.UDFCompatibleDatafeed = function(datafeedURL,symbol,symbolId) {
//Datafeeds.UDFCompatibleDatafeed = function(datafeedURL, updateFrequency,symbolId) {
  ...
  //this._barsPulseUpdater = new Datafeeds.DataPulseUpdater(this, updateFrequency || 2 * 1000 );            
  this._barsWebsocketUpdater = new Datafeeds.DataSocketUpdater(this);  
...
Datafeeds.UDFCompatibleDatafeed.prototype.resolveSymbol = function(symbolName, onSymbolResolvedCallback, onResolveErrorCallback) {
  ...
  if (!this._configuration.supports_group_request) {
     onResultReady(symbolResolveJSON(this._symbol));
  } else {
    if (this._initializationFinished) {
...
//行情流更新数据websocket
Datafeeds.DataSocketUpdater = function (datafeed) {
  this._subscribers = {};
  this._datafeed = datafeed;
}

Datafeeds.DataSocketUpdater.prototype.wsUpdate = function(lastBar,newBar){ 
  var that = this; 
  for (var listenerGUID in that._subscribers) {
    ...
    (function(_subscriptionRecord) { // eslint-disable-line
        ...
        if (!isNaN(_subscriptionRecord.lastBarTime) && newBar.time < _subscriptionRecord.lastBarTime) {

         // console.log('Not New Bar');
          //console.log(isNaN(_subscriptionRecord.lastBarTime));
          //console.log('newbar',newBar.time);
          //console.log('lastbar',_subscriptionRecord.lastBarTime);
          return;
        }
       ...
       // console.log(_subscriptionRecord); 
        ...
        var isNewBar = !isNaN(_subscriptionRecord.lastBarTime) && newBar.time > _subscriptionRecord.lastBarTime;  
...
Datafeeds.DataSocketUpdater.prototype.unsubscribeDataListener = function(listenerGUID) {
  this._datafeed._logMessage('Unsubscribing ' + listenerGUID);
  delete this._subscribers[listenerGUID];
};

Datafeeds.DataSocketUpdater.prototype.subscribeDataListener = function(symbolInfo, resolution, newDataCallback, listenerGUID,onResetCacheNeededCallback) {
  ...
  this._subscribers[listenerGUID].listeners = newDataCallback;  
  //console.log("subscribeDataListener-----listenerGUID-----------");
...
Datafeeds.UDFCompatibleDatafeed.prototype.getBars = function(symbolInfo, resolution, rangeStartDate, rangeEndDate, onDataCallback, onErrorCallback) {
  var params = {
    ...
    from : rangeStartDate,
    to: rangeEndDate,
    //period: transformTime(resolution),
    period: resolution,
    send_period:resolution,
    onDataCallback: onDataCallback,
    onErrorCallback: onErrorCallback
  };
  //console.log('getBars');
  datafeedUtil.websocketGetData(params);

//测试


/*var allKline = setRandomData(from,to);
var historyKine =  dealKlineData(allKline,to); 
console.log('historyKine',historyKine); 

var bars = historyKine.bars,meta = historyKine.meta; 
  ...
  /*var resp = {action:"updateKLine1",data:[
    //{time:from*1000,close:20, open:16, high:23, low:14, volume:4},
    {closePrice:1.2,hightPrice:1.2,klineTime:1525855680,legalId:5,lowPrice:1.2,openPrice:1.2,totalNum:13,type:'1min'}
  ]};

  var bars = transHistoryBars(resp);
  var meta = {no_data:false};
  console.log('----------------------------------------------------------bars');
  console.log(bars);
...
Datafeeds.UDFCompatibleDatafeed.prototype.subscribeBars = function(symbolInfo, resolution, onRealtimeCallback, listenerGUID, onResetCacheNeededCallback) { 
  //var newBarString = localStorage.getItem('dealSuccess');
  //var newBar = JSON.parse(newBarString); 
  //console.log('newBar------------------------');
  //console.log(newBar);  
 ...
 //window.hasWsMessage = '';  
  ...
  this._barsWebsocketUpdater.subscribeDataListener(symbolInfo, resolution, onRealtimeCallback, listenerGUID, onResetCacheNeededCallback);
}; 

Datafeeds.UDFCompatibleDatafeed.prototype.unsubscribeBars = function(listenerGUID) {
  //this._barsPulseUpdater.unsubscribeDataListener(listenerGUID); 
  this._barsWebsocketUpdater.unsubscribeDataListener(listenerGUID);
};

Datafeeds.UDFCompatibleDatafeed.prototype.calculateHistoryDepth = function(period, resolutionBack, intervalBack) {
  switch(period){
    case "1" :return {resolutionBack: 'D', intervalBack: 1};break;                        
    case "5" :return {resolutionBack: 'D', intervalBack: 2 };break;    
    case "15" :return {resolutionBack: 'D', intervalBack: 3 };break;  
    case "30" :return {resolutionBack: 'D', intervalBack: 5 };break; 
    case "60" :return {resolutionBack: 'D', intervalBack: 10 };break;
    case "120" :return {resolutionBack: 'D', intervalBack: 20 };break;
    case "240" :return {resolutionBack: 'M', intervalBack: 1 };break;
    case "360" :return {resolutionBack: 'M', intervalBack: 2 };break;
    case "720" :return {resolutionBack: 'M', intervalBack: 4 };break;
    case "1D" :return {resolutionBack: 'M', intervalBack: 6 };break;  
    case "1W" :return {resolutionBack: 'M', intervalBack: 12 };break;
    default:break;
  }
...
Datafeeds.DataPulseUpdater = function(datafeed, updateFrequency) {
  ...
  var update = function() {
    ...
    for (var listenerGUID in that._subscribers) {
      ...
      //console.log("----------datesRangeRight---------------------------");
      //console.log(new Date(datesRangeRight*1000));

      //  BEWARE: please note we really need 2 bars, not the only last one
      //  see the explanation below. `10` is the `large enough` value to work around holidays
      var datesRangeLeft = datesRangeRight - that.periodLengthSeconds(resolution, 10);

      //console.log("----------datesRangeLeft---------------------------");
      //console.log(new Date(datesRangeLeft*1000));
      ...
      (function(_subscriptionRecord) { // eslint-disable-line
        that._datafeed.getBars(_subscriptionRecord.symbolInfo, resolution, datesRangeLeft, datesRangeRight, function(bars) {
          that._requestsPending--;
          //console.log("_subscriptionRecord bars----------------------------------------------------------");
          //console.log(bars);
          //  means the subscription was cancelled while waiting for data
          if (!that._subscribers.hasOwnProperty(listenerGUID)) {
            return;
          ...
          if (isNewBar) {
            ...
            for (var i = 0; i < subscribers.length; ++i) {
              subscribers[i](previousBar);//onRealtimeCallback
...
Datafeeds.DataPulseUpdater.prototype.subscribeDataListener = function(symbolInfo, resolution, newDataCallback, listenerGUID,onResetCacheNeededCallback) {
  this._datafeed._logMessage('Subscribing ' + listenerGUID);
  ...
  //this._subscribers[listenerGUID].listeners.push(onResetCacheNeededCallback);             
