<template>
  <div class="consign_records" id="consign_records">
    <div class='header'>
      <p class="title">{{$t("message.om.OrderManagement")}}</p>
      <div class="tab_search">
        ...
        <div class='search'>
            <input v-model="form.bb_coin_name" autofocus="autofocus" class="search_ipt" :placeholder='$t("message.om.Coin")'></input>
            <i> / </i>
            <bk-select :listData='otcLegel' :attribute="otcSelectAttr"  @setSelectData="setSelectData">
            </bk-select>
            <el-button plain class='consign_records_btn' @click="searchClick">{{$t("message.om.Serach")}}</el-button>
        </div>
      </div>
    </div>
    <div class='content'>
      ...
      <ul class='table_body'>
        ...
        >
          <el-table-column prop="consign_time" :label="$t('message.om.OrderDate')" min-width="180" align="left" 
              :formatter="tableTimeFormatter">
          </el-table-column>
          <el-table-column prop="" :label="$t('message.om.Paris')"  min-width="130" align="left"  
                           :formatter="tableParisFormatter">
          </el-table-column>
          <el-table-column prop="trans_direction" :label="$t('message.om.Type')"  min-width="70"  align="left" 
                           :formatter="transDirectionFormatter">
          </el-table-column>

          <el-table-column prop="consign_price" :label="$t('message.om.ooPrice')"  min-width="150" align="left" 
                           :formatter="consignPriceFormatter">
                            
          </el-table-column>
          <el-table-column prop="consign_number" :label="$t('message.om.ooAmount')"  min-width="150" align="left" 
                           :formatter="consignNumberFormatter">
          </el-table-column>
          <el-table-column prop="consign_turnover" :label="$t('message.om.ooTotal')"  min-width="160" align="left" 
                           :formatter="consignTurnoverFormatter">
          </el-table-column>
          <el-table-column prop="trans_number" :label="$t('message.om.Executed')"  min-width="140" align="left" 
                           :formatter="executedFormatter">
          </el-table-column>
          <el-table-column prop="" :label="$t('message.om.Unexecuted')"  min-width="140" align="left" 
      ...
      <div :style="{display:form.status==2?'block':'none'}">
        ...
        >
          ...
          <el-table-column prop="trans_avg_price" label="成交均价"  min-width="140" align="left" 
                           :formatter="transAvgPriceFormatter">
          </el-table-column>
          <el-table-column prop="status" :label="$t('message.om.State')"  min-width="140" align="left" 
      ...
      <div :style="{display:form.status==3?'block':'none'}">
         <el-table :data="dealListData"  :fit="tableFit" highlight-current-row border   style="width: 100%;"
          :row-class-name="tableRowClass" :cell-class-name="cellClass"
         >
          <el-table-column prop="finish_time" :label="$t('message.om.OrderDate')" min-width="200" align="left" 
              :formatter="tableTimeFormatter">
          </el-table-column>
          <el-table-column prop="" :label="$t('message.om.Paris')"  min-width="160" align="left"  
                           :formatter="tableParisFormatter">
          </el-table-column>
          <el-table-column prop="trans_direction" :label="$t('message.om.Type')"  min-width="150"  align="left" 
                           :formatter="transDirectionFormatter">
          </el-table-column>

          <el-table-column prop="trans_avg_price" :label="$t('message.om.Price')"  min-width="150" align="left" 
                           :formatter="transAvgPriceFormatter">
                            
          </el-table-column>
          <el-table-column prop="trans_number" :label="$t('message.om.Amount')"  min-width="150" align="left" 
                           :formatter="transAvgPriceFormatter">
          </el-table-column>
          <el-table-column prop="trans_turnover" :label="$t('message.om.Total')"  min-width="150" align="left" 
                           :formatter="transAvgPriceFormatter">
          </el-table-column>
          <el-table-column prop="fee_number" :label="$t('message.om.Fee')"  min-width="120" align="left" 
                           :formatter="transAvgPriceFormatter">
          </el-table-column>
      ...
      <div class="pagination" :style="{display:listData.length!=0?'block':'none'}">
         <el-pagination
          small
          @current-change="handleCurrentChange"
          :prev-text='$t("message.om.Last")'
          :next-text='$t("message.om.Next")'
...
export default {
  ...
  data(){
    return {
      ...
      total_record:0, //数据总数
      price_precision:0,//价格精度
      number_precision:0,//数量精度
      ...
      //查询参数
      form:{
        otc_coin_name:null, //法币名称
        bb_coin_name:null, //币币名称
  ...
  computed:{
    ...
    //委托明细数据
    /*consignRecordsData(){
      return this.$store.state.common.consignRecordsData;
    },
    //撤销
    orderUntodo(){
      return this.$store.state.common.orderUntodo;
    },
    //详情
    consignDetailData(){ 
      let defaultData = [{finish_time_:0,trans_price:0,trans_number:0,trans_turnover:0,rival_fee:0}]
       return this.$store.state.common.consignDetailData || defaultData;
    },*/
  },
...
<style type="text/css">
  ...
  .consign_records .header{
    height: 80px;
    min-width: 1220px;
    padding: 20px;
    background-color: #1C2029;
  }
  .consign_records .content{
    height: 720px;
    min-width: 1220px;
    padding:16px 20px 0;
    background-color: #161920;
  }
  .consign_records .title{
    font-size: 14px;
    line-height: 20px;
    padding-left: 28px;
    margin-bottom: 40px;
    color: #BAC1DA;
    background: url(../../assets/icon_wtgl.svg) 0 0 no-repeat;
  }
  .consign_records .tab_search{
    padding-left: 28px;
  }

  .consign_records .search{
    float: right;
  }

  .consign_records .search_ipt{
    width: 70px;
    height: 16px;
    padding: 4px 10px;
  ...
  .consign_records_btn.is-plain:focus,.consign_records_btn.is-plain:hover,.consign_records_btn.is-plain:active {
    color: #FFAE36;
 ...
 .consign_records .pagination button:disabled,.pagination li{
    background-color: #161920;
    color: #858CA3;
  }
  ...
  /* elementUI css reset */
#consign_records  .el-tabs__item:hover {
    color: #FFAE36;
  }
#consign_records  .el-tabs__item.is-active {
    color: #FFAE36;
  }
#consign_records  .el-tabs__nav-wrap::after {
    background-color: initial;
  }
#consign_records  .el-tabs__active-bar {
    background-color: #FFAE36;
  }
#consign_records  .el-tabs__item {
    /* padding: 0 46px; */
    line-height: 20px;
    color: #BAC1DA;
    font-weight: 200;
...
#consign_records  .el-tabs{
    width: 480px; 
    height: 40px;
    float: left;
  }

#consign_records  .el-pagination .btn-next,#consign_records  .el-pagination .btn-prev{
    background-color: #161920;
  }
 #consign_records  .el-pager li.active{
    color:#FFAE36;
  }
 #consign_records  .el-pagination .btn-next,#consign_records .el-pagination .btn-prev{
     color:#FFAE36;
 ...
 #consign_records .el-table {
    font-size: 12px;
    color: #858CA3;
}
#consign_records .el-table .cell-class{
  padding: 18px 0;
}

#consign_records .el-table,#consign_records .el-table__expanded-cell {
  background-color: #161920;
}
#consign_records .el-table--border,#consign_records .el-table--group {
  border: 1px solid #161920;
  border-bottom: 1px solid #161920;
  /* border-bottom: 1px solid #1C2029; */
}
 #consign_records .el-table th,#consign_records .el-table tr {
  background-color: #161920;
}

#consign_records .el-table td,#consign_records .el-table th.is-leaf {
  border-bottom: 1px solid #1C2029;
}
#consign_records .el-table--border td,#consign_records .el-table--border th,#consign_records .el-table__body-wrapper .el-table--border.is-scrolling-left~.el-table__fixed {
  border-right: 0;
}
#consign_records .el-table--border::after,#consign_records .el-table--group::after,#consign_records .el-table::before {
  background-color: #161920;
}
#consign_records .el-table thead {
  color: #555E70;
  font-weight: 200;
}

#consign_records .el-table__empty-block:nth-of-type(1){
  display: none;
}

#consign_records .el-table__empty-block{
  padding-top: 68px;
  background: url(../../assets/pic_zwjl_heidi.png) center center no-repeat;
  
}
#consign_records .el-table__empty-text{
  display: inline-block;
  padding-top: 50px;
  padding-right: 6px;
  color: #858CA3;
...
#consign_records .el-table--enable-row-hover .el-table__body tr:hover>td {
  background-color: #343A4C;
  border-right: 0;
}
#consign_records .el-table__body tr.current-row>td{
  background-color: #161920;
}


#consign_records .el-button{
  padding: 0;
  margin-left: 6px;
  text-align: center;
  color: #bac1da;
...
#consign_records .el-button.is-plain:focus,#consign_records .is-plain:hover,#consign_records .consign_records_btn.is-plain:active{
  color: #FFAE36;
  border:1px solid #FFAE36;
  background-color: #1C2029;
}
#consign_records .el-table__expanded-cell{
  padding: 0;
}

#consign_records  .el-table__expand-icon>.el-icon {
  display: none;
  }
